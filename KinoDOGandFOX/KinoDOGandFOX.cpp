...
void KinoDOGandFOX::SaveCinemas() {
...
void KinoDOGandFOX::LoadSessions() {
...
void KinoDOGandFOX::SaveSessions() {
    List<String^>^ lines = gcnew List<String^>();
...
void KinoDOGandFOX::CreateTestData() {
...
void KinoDOGandFOX::SaveAll() {
    SaveCinemas();
    SaveFilms();
    SaveSessions();
}

void KinoDOGandFOX::AddCinema(String^ name, String^ cat, int cap, int halls, String^ state) {
    Cinema^ c = gcnew Cinema();
    c->id = nextCinemaId++;
    c->name = name;
    c->category = cat;
...
List<Cinema^>^ KinoDOGandFOX::GetCinemas() { return cinemas; }

array<String^>^ KinoDOGandFOX::GetCinemaNames() {
    array<String^>^ a = gcnew array<String^>(cinemas->Count);
    for (int i = 0; i < cinemas->Count; i++) a[i] = cinemas[i]->name;
    return a;
}

int KinoDOGandFOX::GetCinemaId(String^ name) {
    for each(Cinema ^ c in cinemas) if (c->name == name) return c->id;
    return -1;
}

String^ KinoDOGandFOX::GetCinemaNameById(int id) {
    for each(Cinema ^ c in cinemas) if (c->id == id) return c->name;
    return "?";
}

void KinoDOGandFOX::AddFilm(String^ title, String^ dir, String^ genre, String^ studio) {
    Film^ f = gcnew Film();
    f->id = nextFilmId++;
    f->title = title;
    f->director = dir;
...
List<Film^>^ KinoDOGandFOX::GetFilms() { return films; }

array<String^>^ KinoDOGandFOX::GetFilmTitles() {
    array<String^>^ a = gcnew array<String^>(films->Count);
    for (int i = 0; i < films->Count; i++) a[i] = films[i]->title;
    return a;
}

int KinoDOGandFOX::GetFilmId(String^ title) {
    for each(Film ^ f in films) if (f->title == title) return f->id;
    return -1;
}

String^ KinoDOGandFOX::GetFilmTitleById(int id) {
    for each(Film ^ f in films) if (f->id == id) return f->title;
    return "?";
}

void KinoDOGandFOX::AddSession(int cid, int fid, String^ date, String^ time, double price, int total, int sold) {
    Session^ s = gcnew Session();
    s->id = nextSessionId++;
    s->cinema_id = cid;
    s->film_id = fid;
...
void KinoDOGandFOX::UpdateCinema(int index, String^ name, String^ cat, int cap, int halls, String^ state) {
...
void KinoDOGandFOX::UpdateFilm(int index, String^ title, String^ dir, String^ genre, String^ studio) {
...
void KinoDOGandFOX::UpdateSession(int index, int cinema_id, int film_id, String^ date, String^ time, double price, int total, int sold) {
...
void KinoDOGandFOX::DeleteCinemaAtIndex(int index) {
    if (index >= 0 && index < cinemas->Count) {
...
void KinoDOGandFOX::DeleteFilmAtIndex(int index) {
    if (index >= 0 && index < films->Count) {
        Film^ f = films[index];
        for (int i = sessions->Count - 1; i >= 0; i--) {
...
void KinoDOGandFOX::DeleteSessionAtIndex(int index) {
    if (index >= 0 && index < sessions->Count) {
...
List<Session^>^ KinoDOGandFOX::GetSessions() { return sessions; }

// ========== ЗАПРОСЫ ==========

List<String^>^ KinoDOGandFOX::GetRepertoireByDate(int cinemaId, String^ date) {
...
int KinoDOGandFOX::GetFreeSeats(int cinemaId, String^ date, String^ time) {
    for each(Session ^ s in sessions)
...
int KinoDOGandFOX::GetSoldTicketsForFilmPeriod(String^ filmTitle, String^ startDate, String^ endDate) {
...
void KinoDOGandFOX::SaveReportToFile(String^ filename, List<String^>^ lines) {
...
void KinoDOGandFOX::LoadCinemasGrid() {
...
void KinoDOGandFOX::LoadFilmsGrid() {
    dgvFilms->Rows->Clear();
    for each(Film ^ f in films) {
...
void KinoDOGandFOX::LoadSessionsGrid() {
...
void KinoDOGandFOX::OnAddCinema(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnEditCinema(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnDeleteCinema(Object^ sender, EventArgs^ e) {
    if (dgvCinemas->SelectedRows->Count == 0) {
...
void KinoDOGandFOX::OnAddFilm(Object^ sender, EventArgs^ e) {
    Form^ form = gcnew Form();
...
void KinoDOGandFOX::OnEditFilm(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnDeleteFilm(Object^ sender, EventArgs^ e) {
    if (dgvFilms->SelectedRows->Count == 0) {
...
void KinoDOGandFOX::OnAddSession(Object^ sender, EventArgs^ e) {
    if (cinemas->Count == 0) {
...
void KinoDOGandFOX::OnEditSession(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnDeleteSession(Object^ sender, EventArgs^ e) {
    if (dgvSessions->SelectedRows->Count == 0) {
...
void KinoDOGandFOX::OnShowRepertoire(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnSaveRepertoire(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnShowFreeSeats(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnSaveFreeResult(Object^ sender, EventArgs^ e) {
    SaveFileDialog^ dlg = gcnew SaveFileDialog();
    dlg->Filter = L"Текстовые файлы|*.txt";
    if (dlg->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
        List<String^>^ lines = gcnew List<String^>();
...
void KinoDOGandFOX::OnShowSoldTickets(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::OnSaveSoldReport(Object^ sender, EventArgs^ e) {
    SaveFileDialog^ dlg = gcnew SaveFileDialog();
    dlg->Filter = L"Текстовые файлы|*.txt";
    if (dlg->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
        List<String^>^ lines = gcnew List<String^>();
...
void KinoDOGandFOX::OnExit(Object^ sender, EventArgs^ e) {
...
void KinoDOGandFOX::InitializeComponent() {
...
void KinoDOGandFOX::OnFormResize(Object^ sender, EventArgs^ e) {
    if (btnExit != nullptr) {
